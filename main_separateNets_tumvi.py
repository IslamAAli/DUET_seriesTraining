...
test_seqs = ["dataset-room1_512_16",
             "dataset-room3_512_16",
             "dataset-room5_512_16",
...
corrected_imu_base_path = base_path+'/data/tumvi_corrected'
...
lr = 0.01
num_epochs = 1800
ckpt_path_gyro = base_path+'/models/tumvi_gyro.pt'
ckpt_path_acc = base_path+'/models/tumvi_acc.pt'
...
if ENABLE_GYRO_NET:
    ...
    # ===========================
    # Gyro Net
    gyro_net = IMUNetGyro(in_channel, layer_channels, out_channel, kernel_size, dropout, tumvi.mean, tumvi.std).cuda()
    ...
    net = IMUNetGyro(in_channel, layer_channels, out_channel, kernel_size, dropout, tumvi.mean, tumvi.std).cuda()
    net.load_state_dict(torch.load(ckpt_path_gyro))
    net.eval()
    # running_time = test_model(net, metric_dict_test, euroc.test_seqs)

    get_predictions(net, tumvi.test_seqs, True, corrected_imu_base_path)   # predictions[0] => correction in gyro, predictions[2] => corrected gyro readings

    running_time = test_gyro_model(net, metric_dict_test_gyro, tumvi.test_seqs)
...
if ENABLE_FIXED_DATA_PROPAGATION:
    ...
    # =================================================================== 
    source_folder =  base_path+"/data/tumvi"

    # Destination folder path
    destination_folder = base_path+"/data/tumvi_fixed_gyro_raw_acc"
    ...
    # ===================================================================
    base_folder = base_path+"/data/tumvi"
    corr_folder = base_path+"/data/tumvi_corrected"
    dst_folder  = base_path+"/data/tumvi_fixed_gyro_raw_acc"
...
if ENABLE_ACC_NET:
    ...
    # ===========================
    # Acc Net
    acc_net = IMUNetAcc(in_channel, layer_channels, out_channel, kernel_size, dropout, tumvi.mean, tumvi.std).cuda()
    ...
    metrics_for_early_stopping_acc = ['AOE']
    ...
    net = IMUNetAcc(in_channel, layer_channels, out_channel, kernel_size, dropout, tumvi.mean, tumvi.std).cuda()
    net.load_state_dict(torch.load(ckpt_path_acc))
    net.eval()
    # running_time = test_model(net, metric_dict_test, euroc.test_seqs)

    get_predictions(net, tumvi.test_seqs, False, corrected_imu_base_path)   # predictions[1] => correction in acc, predictions[3] => corrected acc readings

    running_time = test_acc_model(net, metric_dict_test_acc, tumvi.test_seqs)
    print(f'running time: {running_time}')
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
